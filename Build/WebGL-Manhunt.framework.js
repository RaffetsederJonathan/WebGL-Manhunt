...
// end include: preamble.js
  ...
  function _StartWebGLGeolocation(gameObjectNamePtr, callbackMethodNamePtr) {
          ...
          function sendToUnity(msg) {
              try {
                  if (typeof unityInstance !== "undefined") {
                      unityInstance.SendMessage(gameObjectName, callbackMethodName, msg);
                  } else if (typeof SendMessage !== "undefined") {
                      SendMessage(gameObjectName, callbackMethodName, msg);
                  }
              } catch (e) {
                  console.error(e);
              }
          ...
          if (!window.isSecureContext) {
              sendToUnity("ERROR:NOT_HTTPS");
              return;
          }
  
          if (!navigator.geolocation) {
              sendToUnity("ERROR:NOT_SUPPORTED");
              return;
          }
          ...
          function onError(error) {
              ...
              switch (error.code) {
                  case error.PERMISSION_DENIED:
                      sendToUnity("ERROR:CODE_1");
                      break;
                  case error.POSITION_UNAVAILABLE:
                      sendToUnity("ERROR:CODE_2");
                      break;
                  case error.TIMEOUT:
                      sendToUnity("ERROR:CODE_3");
                      break;
                  default:
                      sendToUnity("ERROR:UNKNOWN");
                      break;
              }
          }
  
          // Alten Watch entfernen, falls bereits einer läuft
          if (window.unityGeoWatchId !== undefined) {
              navigator.geolocation.clearWatch(window.unityGeoWatchId);
              window.unityGeoWatchId = undefined;
          }
  
          // Direkt watchPosition starten (fordert Pop-up an, liefert Sofort-Position & beobachtet weiter)
          window.unityGeoWatchId = navigator.geolocation.watchPosition(
              onSuccess,
              onError,
              {
...
var wasmImports = {
  ...
  "StartWebGLGeolocation": _StartWebGLGeolocation,
